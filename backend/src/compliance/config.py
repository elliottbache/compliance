...
class Settings(BaseSettings):
    ...
    def _validate_envs(self) -> "Settings":
        ...
        if self.app_env in ["staging", "production"]:
            ...
            if any(
                origin in ["http://localhost:5173", "*"]
                for origin in self.allowed_cors_origins
            ):
                raise ValueError(
                    "For production and staging environments, CORS origins should not include localhost or *.  Set this in .env file.  Check /etc/compliance/.env."
                )
    ...
    def allowed_cors_origins(self) -> list[str]:
        """Return configured CORS origins parsed from a comma-separated string."""
        return [
